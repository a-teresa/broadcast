...
#if DT_NODE_HAS_STATUS(DT_NODELABEL(uac2_headphones), okay)
		dev = DEVICE_DT_GET(DT_NODELABEL(uac2_headphones));
		LOG_INF("USB initialized as unidirectional (headphones only).");
#else
		LOG_ERR("uac2_headphones is disabled in devicetree for this build.");
		return -ENODEV;
#endif
	} else if (host_in) {
		/* No mic-only UAC2 descriptor exists, so reuse the bidirectional one.
		 * The headphones-out terminal it exposes is left undriven.
		 */
		dev = DEVICE_DT_GET(DT_NODELABEL(uac2_headset));
		LOG_INF("USB initialized as unidirectional (microphone only).");
	} else {
