...
/* Gateway sends its (bidirectional) uplink over USB; headset can send its decoded
 * downlink over USB instead of I2S/HW codec.
 */
#define AUDIO_DEV_USES_USB                                                                       \
	(((CONFIG_AUDIO_DEV == GATEWAY) && (CONFIG_AUDIO_SOURCE_USB)) ||                          \
	 ((CONFIG_AUDIO_DEV == HEADSET) && (CONFIG_AUDIO_SINK_USB)))
...
/* Used when audio is sent to the USB host instead of I2S/HW codec: on gateway for the
 * bidirectional uplink, and on headset when CONFIG_AUDIO_SINK_USB is enabled.
 */
int audio_system_decode(struct net_buf *audio_frame_in)
{
	...
	*meta_out = decoder_meta;
	...
	/* decoder_meta hardcodes mono; reflect the decoder's actual channel count
	 * (e.g. stereo headset-USB-sink) instead of always forcing 1 channel out.
	 */
...
int audio_system_init(void)
{
	...
	if (CONFIG_AUDIO_DEV == GATEWAY) {
		host_in = IS_ENABLED(CONFIG_STREAM_BIDIRECTIONAL);
		host_out = true;
	} else {
		/* Headset: send decoded downlink audio to the host, no playback from host. */
		host_in = true;
		host_out = false;
	}
